# online_rit — 结果报告（2026-09-15 03:40 CDT）
...
图：`figures/sr_ir_libero_spatial.png`（点数据 `figures/sr_ir_libero_spatial.json`；灰点为 R 线 84 臂历史背景，取自 `exp/rit_pareto/analysis/figures/groot_libero_spatial.json`）。新方法各臂与 R′ 全部落在 IR 64–80 的同一簇；R 线背景在 IR≈50 已有 SR 0.93 的点，新阶梯到不了那里——去掉 FULL_HIT 后最浅档 warm@0.875 已付 MISS 的 40.7%，在线规则的回放 IR 下限为 62%。
...
6. **未做**：libero_10 闭环（M1 FAIL）、目标 90 / 60 / 50（共同工作点不足）、spatial 中位目标的第三组重复（计划要求两组配对重复，本次只有 @80 与 FM-0@70 各一组额外流）；成本台账按 D19 不实测。
