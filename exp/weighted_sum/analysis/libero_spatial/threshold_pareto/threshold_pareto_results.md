...
| file | content |
|---|---|
| `data/libero_spatial/threshold_pareto/d{1,3,4,5}/journal.jsonl` | per-depth per-episode terminal records (8,300 each; 33,200 total) |
| `data/libero_spatial/threshold_pareto/d{1,3,4,5}/eval_per_step.jsonl` | per-depth per-step records (hit_type + cp1_score; 818,994 total) |
| `data/libero_spatial/threshold_pareto/d{1,3,4,5}/results.json` | per-depth per-yaml SR (83 entries each; 332 total) |
| `data/libero_spatial/threshold_pareto/d{1,3,4,5}/inf_ratio.json` | per-depth per-yaml `(inference_ratio, n_FH, n_WS, n_MISS)` |
| `data/libero_spatial/threshold_pareto/warmup_per_step.jsonl` | 8,695 cp1_score values from the 400-ep warmup |
| `data/libero_spatial/threshold_pareto/per_base/<base>__warmup.jsonl` | warmup scores split per base for per-base threshold solving |
