...
3. **可动物体·类别重抽 = 4 个死任务**（PickPlace 族 0.011–0.017，1056 集/任务终数，总成功 12–18 次）：每集从任务物体池重抽类别+位置+干扰物；**n=5 库只覆盖 eval 类别的 19%（6/32）**；成功=类别巧合×几何巧合（CounterToStove 的 7 次成功全在库中不存在的"鸡蛋"上——纯几何巧合）。教师采集期同场景 0.60–0.82（n=26–48）⇒ **不是任务超纲，是纯 cache + 小库的结构性迁移失败**。裁决：从搜索指标中剔除（分析层双口径），定性为方法失败模式证据。

**正面发现**：CloseFridge 上纯 cache **反超教师**——教师采集期同场景 36/83=0.43，cache 均值 0.481、好权重区 0.75–0.88（≈2×）；TurnOnSinkFaucet 弱化版（教师 24/100=0.24，cache 均值持平、好权重区 0.50≈2×）。重放已验证成功的轨迹规避了教师现场推理的失败模式；cache 的价值区与失效区在本 benchmark 上被干净地划开。（教师基线口径=采集战役同场景实测 n=26–100；准入门 n=5 切片粒度太粗曾致误引，见解剖报告 §3 列定义。）
