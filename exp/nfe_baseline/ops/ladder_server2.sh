#!/bin/bash
# Server-side lane driver v2 (h100 / weilandserver): serve every k of one (policy, suite) in order, switching k on the
# client's explicit DONE signal (kdone_listener.py touches /tmp/nfe/kdone_<policy>_<k>); the idle timeout is only a fallback.
# A k whose server is already up (nfesrv<port> claimed) is adopted, so this ladder can replace a running v1 ladder mid-k.
#
