#!/bin/bash
# weilandserver (t108 cannot init CUDA: cuInit 101, a 4th A5000 is missing from nvidia-smi): official Diffusion Policy (real-stanford/diffusion_policy) environment via micromamba (no sudo on the box):
...
[ -e $D/env/lib/libGL.so ] || ln -s /usr/lib/x86_64-linux-gnu/libGL.so.1 $D/env/lib/libGL.so
[ -e $D/env/lib/libEGL.so ] || ln -s /usr/lib/x86_64-linux-gnu/libEGL.so.1 $D/env/lib/libEGL.so
$D/env/bin/pip install "huggingface_hub==0.20.3" 2>&1 | tail -1
# kitchen env (adept_envs) needs dm_control; the mujoco wheel must be pinned (newer ones try to build from source on py3.9)
$D/env/bin/pip install "mujoco==2.3.7" 2>&1 | tail -1; $D/env/bin/pip install "dm-control==1.0.9" 2>&1 | tail -1
echo "== $(date +%H:%M:%S) mujoco-py compile check"
export PATH=$D/env/bin:$PATH MUJOCO_PY_MUJOCO_PATH=$D/mujoco210 LD_LIBRARY_PATH=$D/mujoco210/bin:$D/env/lib:/usr/lib/nvidia:${LD_LIBRARY_PATH:-} CPATH=$D/env/include LIBRARY_PATH=$D/env/lib MUJOCO_GL=egl PYOPENGL_PLATFORM=egl
