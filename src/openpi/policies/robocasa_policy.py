"""RoboCasa365 input/output transforms for the pi0.5 teacher.

Rescued verbatim from the previously untracked copy on weilandserver
(``/home/weiland/openpi/src/openpi/policies/robocasa_policy.py``, itself ported
from the robocasa-benchmark/openpi fork). Provenance and the source sha256 are
recorded in ``exp/robocasa365/baselines/PROVENANCE.md``. Apart from this
docstring and trailing-whitespace cleanup, the module is unchanged so the
transform chain stays byte-equivalent to the one the RoboCasa365 admission
gates ran through.

Deliberately free of any ``robocasa`` import: it only needs einops / numpy /
openpi.transforms, so registering a config that uses it does not pull the
py3.12 sim island's dependencies into the main venv
(``tests/robocasa365/test_robocasa_policy_config.py`` pins this).
"""
