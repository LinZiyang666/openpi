% Technical note for collaborators: risk-indexed thresholds for cached dispatch
% (the s-only pivot). Companion to dispatch_note.tex and redundancy_note.tex.
...
\theta_a\})$. Every policy discussed in this note---the hand-tuned threshold
dispatch, its percentile-indexed grid, and the calibrated rule---is a member
of this class at run time; a deployment differs from the incumbent only in
\emph{how $\theta$ is chosen offline}. This note makes that difference the
object of study.

\paragraph{Two indexings of one family.}
The incumbent \emph{rate indexing} sets each $\theta_a$ at a percentile of
...
so the incumbent finds operating points by rolling out a $K$-dimensional
grid. The alternative studied here is \emph{risk indexing}. Let $Y_a \ge 0$
...
the incumbent: the same comparisons against stored constants.
...
cut, so
$\{\text{risk-indexed rules}\}_{\delta} =
\{\text{threshold rules}\}_{\theta} =
...
(Neyman--Pearson). At $K=1$, therefore, risk indexing buys semantics but no
...
for all $a$, which is exactly Eq.~\ref{eq:cuts}. The calibrated ladder
$\{\,\theta(\delta) : \delta \ge 0\,\}$ is thus the plug-in estimate of the
solution path of the $K$-dimensional design problem, and a $K$-dimensional
grid search rediscovers that path pointwise. Two consequences are testable:
(i) ladder points should track the grid family's empirical Pareto hull at
matched evaluation budget; (ii) at a fixed number $n$ of a-priori arms, the
vertical regret of a $K$-dimensional grid against its own hull grows with
$K$, while the ladder remains one-dimensional. Neither consequence asserts
that the path \emph{dominates} the grid family---it cannot, being a subset;
the claim is that the path attains the family's frontier without the search.

\paragraph{Per-task and cross-library transfer.}
Rate indexing conditions on nothing: a percentile vector tuned on one task
mixture or one library encodes that mixture's score distribution, and the
incumbent's own documentation records that carrying a threshold across
libraries is a known failure mode. Risk indexing recomputes
...
property of the information set, not of any single rule family; what risk
indexing changes is the \emph{cost of exploiting it}.
...
risk ladder against the dense percentile grid on the same evaluation set,
...
each indexing; the prediction is a widening gap as arms are removed, and a
...
---the plot only a risk-indexed rule can draw in advance. (iv) \emph{System
composition}: the full deployed stack adds the production hysteresis gate in
front of either indexing; the ceiling-equality reading predicts that gate
plus risk ladder matches gate plus tuned grid at matched freedom, measured
...
$3$-dimensional grid against the unchanged one-dimensional ladder.
