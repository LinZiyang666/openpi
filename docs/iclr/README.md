...
| [latex/](latex/) 合作者材料五件套 | ICLR 模板英文 note（各配 PDF）：**sonly_note**（RIT vs GST：同一阈值规则族的两种离线索引、K=1 等价让步、单乘子解路径、per-task/跨库迁移、四条实验推论）；**redundancy_note**（R(ε) 全量形式化 + 实验呼应逐条）；**dispatch_note v3**（免训练分级调度：代理事件 NP 正名 + 耦合噪声可执行协议 + $\tau=0$ fallback 与有限样本条件 + episode 级同时共形覆盖 + split 职责/coverage 口径/start_t 约定，两轮外审修订）；**experiment_list**（E0–E13 状态表，E1/E2 同图成组）；**paper_outline**（提纲 v0.2 英文版含 E13 真机节） |
