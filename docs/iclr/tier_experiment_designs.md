...
**Init 三池协议（污染红线，owner 2026-08-15 定）**：测试集只许测量、不许拟合。"拟合"类活动（库构建/学生蒸馏/router 训练标签/阈值标定/任何超参与信号选择）一律限制在训练侧池；"测量"类活动（各臂 SR、各信号 AUROC 评估）允许在 eval 集上进行。三池：**A = 官方 eval（500/套件，硬约束不可扩，任何 fit 触碰即作废）**；**B = 蒸馏/库池（现有 500 差集池，已消耗，与 A 零交集已验证）**；**C = 标定/router 池（按需新铸——LIBERO init 可从任务分布采样，项目已有先例：差集池/db_init 池/markov 线第二池 + `WorkerSpec.init_states_dir` 透传——与 A、B 均不相交）**。router 协议 = 在 C 池学生 rollout 上训练、在 A 池 shadow rollout 上评估（复刻部署语义）。附带论据（入 Q1）：router 的训练标签只能长在学生已见分布附近——恰是它最少失败处；router 最需要准的新 init 上恰好零标签——标签经济学的"位置"问题，叠加在"数量"问题之上。
...
- 设计：**shadow 模式**跑纯学生闭环（search+judge 照常、派发关闭、恒执行学生）各 500 ep/套件。**数据协议（三池）**：AUROC 评估用的 shadow rollout 跑在 A 池（官方 eval init，纯测量）；trained-router 的训练标签来自 **C 池**（新铸标定池）的独立学生 rollout——训练在 C、评估在 A，任何信号/基线不得在 A 上拟合或调参。逐步记录：本方信号（cp1_score/margin/winner 持久性）+ 基线信号：kNN-OOD 距离（学生特征空间对学生训练集）、STAC 式动作时序一致性（相邻 chunk 重叠段距离）、SmolVLA 采样方差（K=8）/ACT 用 3-seed ensemble 分歧（仅一套件，控成本）+ **trained-router 基线 = MC value 式成功预测器**（教授/审稿人必问"为何不训个小模型当 router"的实证回应，同时就是 RL 界/ThriftyDAgger 的标准答案——从学生 rollout 的 episode 结局拟合成功概率，MC/TD 自动做 credit assignment，无需逐步标签：小 MLP probe 吃同一内部特征、以 held-out 学生 shadow rollout 的失败结局为标签训练；报 **label-efficiency 曲线**——AUROC 随标注 episode 数 {25,50,100,200,400} 变化，与零标签 index 信号的水平线对比，交点即"多少标注预算下 index 是正确选择"，亦即给 RL 方案标价。IC-Cache 式 bandit 路径不作基线：其前提——免费逐决策反馈——在闭环控制中不存在，正文 §3.2 论证即可。**可选 P2 补充：在线 RL router 学习曲线**——router 作为 RL 策略在 sim 闭环内训练（reward = 成功 − λ·teacher 调用，轻量 contextual-bandit/REINFORCE，~1000–2000 ep），性能 vs 累计交互 episode 数画进同一张图（与离线 label-efficiency 曲线共享 rollout 货币 x 轴）；预期低预算区离线版更强（无 explore 学费）、高预算区在线版或反超（吃 visitation-shift 自适应红利）——主张区间恰在低预算区，一图同时回答两种"训 router"）。**标签协议**（预注册）：episode 失败为结局；step 级 AUROC 按 lead-time 分层（距 episode 结束 ≥k 步的窗口单列；失败 episode 末段 X% 剔除或单列为"事故后检测"）；推断单位 = episode（cluster bootstrap）；所有基线共用同一标签同一聚类。**teacher-failure 对照**：同法在 teacher-only rollout 上预测 teacher 失败。
...
- 产出：SR × 实测 GPU-s/step 散点 + Pareto（normalized 与裸 GPU-s 双 panel）+ savings-ceiling inset（X11）+ Table 1（预注册 OP × SR/成本/三层占用率/驻留显存）。
