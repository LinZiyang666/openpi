...
├── data_collection/  # Data collection (HDF5 schema, --trace-out trace serving mode)
...
| [experiments/actioncache_baseline.md](experiments/actioncache_baseline.md) | ActionCache 式 post-backbone（CP2）基线 runbook：CP1 库 → CP2 库（backbone 输出稀疏三值投影 500 维，id/payload/链边一一对应）→ 验证器 + parity 门 → shadow 表 GST K=1 IR 寻址切点 → 臂 yaml（N_hit∈{0,1} = FULL_HIT / WARM@0.1）→ `run_gtp.py --checkpoint cp2` → 开销实测 → 聚合与 `exp/rit_pareto` 前沿对照；§9 GR00T N1.5 × LIBERO 同库组（`cp2_groot_ternary`，WARM@0.875，E 计入分子，岛上重建建库 / 采集验收 / E+preflight 门，10 臂/组）。设计 `logs/archive/actioncache_baseline_plan.log.md` 与 `logs/actioncache_baseline_groot_plan.log.md`，代码 `exp/actioncache_baseline/` + `exp/libero_groot/{cp2_reconstruct,build_cp2_artifact_groot,build_shadow_table_groot,groot_cp2_parity,verify_shadow_h5,emit_task_map,bench_cp2_overhead_groot}.py`；G2 R2 修复：共享 teacher 表/每 suite E 身份分离、150 集实际覆盖和测量值检查、固定十臂 ID |
...
| File | Description |
|------|-------------|
| [data_collection/guide.md](data_collection/guide.md) | HDF5 data collection via the trace serving mode (`--trace-out` / `--trace-build-cache`, superset of the legacy `--collect` file, commit protocol, whole-batch trace admission + fatal drain rules), schema and directory layout; **gate-research per-step collection** (`--collect-gate-dir` / `--export-collect-meta`, `__collect_meta__`, per-step verdict + `episode_summary` row schema, conductor `extra` contract, `summarize_gate_log`); **RoboCasa365 teacher-library collection** (one-server-one-worker topology, run-plan artifact, audit + deterministic manifest, binomial completion rule); **pinned-object task variants** (`--pinned-objects`, realized-provenance admission); **denoise-schedule stamp** (`denoise_schedule_id` / `denoising_num_steps` file attrs, `noise_action_0` init noise, GR00T snapshots via the `action_encoder` hook, `--require-denoise-schedule` audit) |
