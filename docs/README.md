...
| File | Description |
|------|-------------|
| [cache/warm_reset_experiments.md](cache/warm_reset_experiments.md) | Warm reset 实验入口：任务清单、YAML 生成、concurrent server + conductor 多臂、LIBERO / RoboCasa 标准 worker（含 conda / venv 两种模拟器环境的 agent 用法）、三路证据准入与实测 NFE 汇总 |
