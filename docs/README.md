...
| [experiments/cache_latency_bench.md](experiments/cache_latency_bench.md) | Cache CP1 `check()` 六段延迟**回放基准** runbook：不加载模型、按真实 cache yaml 组装真 `CacheOrchestrator`、用 H5 真实 trajectory 逐 step 回放驱动 cache 真实工作、`SystemTimer` 探针记录每请求每部件延迟（per-step CSV + 聚合 json）；支持 threshold/always_hit/always_warm_start/composite(offline-calib + enriched pkl)；已知偏差：CPU build 段无 D2H / 仅驱动 CP1 / 读 action-history 的 composite judge 非逐调用等价。代码 `exp/cache_latency_bench/`，设计 `logs/cache_latency_bench_plan.log.md` |
