...
> Version: 0.6
> Status: Implemented — CP1 three-level judgment (FULL_HIT / WARM_START / MISS), CP2 active as the ActionCache-style post-backbone single-key checkpoint (`cp2_vlm_ternary` on Pi0.5, `cp2_groot_ternary` on GR00T N1.5; CP2-only configs; warm start remains at CP1 for the main line)
...
#### CP2 on GR00T N1.5 (LIBERO) — `cp2_groot_ternary`

> Second teacher of the same baseline (`logs/actioncache_baseline_groot_plan.log.md`; runbook `docs/experiments/actioncache_baseline.md` GR00T 章). Same protocol (single key, top-1 cosine, whole-suite, `write_policy: never`, two N_hit tiers), different key source and cost bookkeeping. The GR00T split lives on the torch-only island (§5.17) and nothing here imports jax.

- **Key source (paper §4.1 / Table 5)**: the action head's *encoded* conditioning, not the raw backbone output. `GrootStagedRunner.run_cp2_key_source(stage2) -> GrootCP2KeySource(vl_encoded [N, 2048], state_encoded [1536])` runs `head.process_backbone_output` (vlln LayerNorm + `vl_self_attention`) and `head.state_encoder(state, embodiment_id)[0, -1]` once, on a fresh `BatchFeature` so the in-place normalisation never writes back into `stage2`. The head must be in eval mode; the tensors are detached views that may be inference tensors and are released by `clear()` within the same decision (R2-B11). The helper is *not* folded into `run_stage3` / `run_stage3_from` (those keep upstream `get_action` / the pinned transcription byte-for-byte), so a WARM_START / MISS runs the two encoders twice — that second forward is the `E` term below, timed by the `cp2_encode` probe.
- **Key builder** `cp2_groot_ternary` (`cache/groot/cp2_key_builder.py`): layout `groot_encoded_v1` — the `[N, C]` encoded tokens zero-padded on the token axis to `token_len` (default 640; LIBERO N≈566), flattened, then the encoded state appended: `D = 640·2048 + 1536 = 1,312,256`; a sequence longer than `token_len` is a hard error. Projection is the Pi0.5 implementation (`get_projection_spec` / `project`, float32 accumulation) with `d=500, p=0.01`; `projection_meta()` = the Pi0.5 spec fields **plus** the `layout` block, and the artifact binding compares the full dict both ways (a Pi0.5-shaped `projection` without `layout` is refused). `build()` materialises the key under `inference_mode(False)`, CPU float32, so the storage contract holds even though the source is an inference tensor.
- **Interceptor** (`cache/groot/interceptor.py::_get_action_cp2`): one session for stage 1 → `run_stage2_llm` → `run_cp2_key_source`; exactly one `check(CP2, stage2=, cp2_source=)` outside the session (the backend cosine must not run under the session's autocast); FULL_HIT replays the chunk, WARM_START resumes `run_stage3_from(stage2, snapshot, 0.875, schedule=<library schedule>)`, MISS runs `run_stage3(stage2)` (upstream `get_action`, same numbers as `run_stage2`). `cp2_only` is frozen at construction from `orchestrator.has_checkpoint(CP2)`; the CP1 cycle is untouched. Prologue / Euler-step counts: FULL 1 / 0, WARM@0.875 2 / 1, MISS 2 / 8.
- **Guards** (`cache/groot/load_guard.py`): the serviceable checkpoint set is exactly `{cp1}` **or** `{cp2}`; a `cp2` recipe must name `key_builder.type: cp2_groot_ternary` (the Pi0.5 `cp2_vlm_ternary` reads `Stage2Output.prefix_out`, which the split does not produce); warm-start / schedule rules read the enabled checkpoint instead of a fixed `cp1`; a `cp2` recipe must name its `denoise_schedule` and match the live head for **both** tiers (an N_hit=0 arm's MISS is still the k=8 teacher its library and cost table describe); `validate_artifact_identity` expects `checkpoint_id == "CP2"` and the artifact's `schedule_id` / `denoising_num_steps` to be the recipe's under a cp2 recipe; the hysteresis opt-in stays CP1-only. `config.py` gained `CP2GrootKeyBuilderConfig` (`key_builder.cp2_groot`: seed / d / p / token_len / feature_dim / state_feat_dim), the CP2 builder type set `{cp2_vlm_ternary, cp2_groot_ternary}` in R-CP2, `cp2_expected_projection_meta(config)` as the single source for both the online binding and the offline verifier, and the factory branch. `serve_groot_libero.py` admits the builder alongside the two-camera CP1 builders; bundles hot-swapped under one id resolve to whatever is registered at that moment, and every connection binds the interceptor it was built with.
- **Task identity**: a LIBERO task has two names — `task.name` (canonical, underscore-joined, locates the `.init` pool; the shadow manifest's `task_name`) and `task.language` (the instruction the client sends on `episode_start` and the collector stores as the H5 `task` attr). The cohort verifier binds them through `task_id` with a map written from the benchmark (`exp/libero_groot/emit_task_map.py`) and never derives one from the other; templates are built from the instruction.
- **Offline path**: W13 H5 files hold no raw images, so the CP2 library is built by *reconstruction* (`exp/libero_groot/cp2_reconstruct.py`, `stage1_path = groot_reconstructed_template`): a zero-image template stage-1 sequence per instruction, its image runs overwritten from `vision_0/1` (fp16) and the valid state slots from `robot_state`, asserting the template's text positions equal the stored `prompt_emb` and the state reads back exactly. `build_cp2_artifact_groot.py` copies every W13-S3 entry (id / payload / chain edges) and only replaces the key; the artifact carries `schedule_id`, `teacher`, `stage1_path`, `projection` (with `layout`), `model.weights_digest`, `h5_manifest`, `source_pkl_sha256`.
- **Cost bookkeeping** (`exp/actioncache_baseline/libs.py::CostRecord`, formula `groot_cp2_encoded_additive_v1`): `P = s1 + s2`, `L` = full 8-step loop, `M = P + L` (the teacher, IR denominator, no `E`); FULL = `P + E`, WARM@0.875 = `P + E + L/8`, MISS = `M + E`, so an all-MISS arm prices above 100 %. `E` is measured per suite (`bench_cp2_overhead_groot.py --mode encoder-cost`, CUDA-Graph certified against an nsys trace, bound by content to the shared teacher table, its GPU / frozen sampling, and this suite's library `weights_digest`; the table's calibration checkpoint and each suite's encoder checkpoint are retained separately) and is never defaulted to zero; the export refuses without a bound `E` record, a complete shadow-table sidecar plus actual 10 × 15 episode coverage with unique contiguous decisions and finite scores, and a decision-overhead preflight with finite nonnegative measurements whose verdict is recomputed from the warm P95. Aggregation validates provenance values, binds the encoder model/layout to the exported library, and recomputes the denominator and verdict prices. Both NFE tiers must appear exactly once in the ten-arm matrix; fixed `t01`–`t04` target IDs prevent fractional IR targets from overwriting each other.
...
| Type | Behavior |
|------|----------|
| `threshold` | FULL_HIT if `score >= threshold`. With `warm_tiers` configured, scores below the threshold are matched against descending tiers for WARM_START (CP1 only). Returns `JudgeResult(hit_type, winner_id, start_t)`. In the dispatch-surface line, a `threshold` judge whose cuts are picked by rolling out a grid of development-score percentiles is the **GST (Grid-Searched Threshold)** baseline. |
...
| `dispatch_surface` | (Dispatch-surface line; the **RIT — Risk-Indexed Threshold**: same online rule class as `threshold`, cuts computed offline from calibrated risk curves at one δ instead of searched) Three-tier verdict — FULL_HIT / WARM_START(start_t=0.3) / MISS — from two conformally calibrated monotone boundaries over `(s, v)`: `s` = fused similarity (`results[0].score`), `v` = weighted top-k action disagreement computed from candidate payloads via `PayloadView.get_many`. Everything lives in an NPZ artifact (`surface_artifact_path` is the only YAML field): k, W, δ, boundaries, plus a `retrieval_contract` compared at load (yaml digests) and at assembly (library sha256 / entry count / action schema / t=0.3 completeness via the identity-extended `artifact_meta`). `uses_disagreement=false` is the s-only nested-ablation mode (no payload fetch, no v). Fail-closed: empty results, non-finite s/v, missing view, K_eff<2, or v beyond the fitted support all yield MISS. `min_required_top_k` lifts the search width to the artifact's k. CP1-only; requires frozen preload + `write_policy: never`. See `src/openpi/cache/components/surface_judge.py` and [`logs/archive/dispatch_surface_plan.log.md`](../../logs/archive/dispatch_surface_plan.log.md). |
...
> **Source**: `src/openpi/cache/config.py` (`CollectionConfig`), `src/openpi/cache/interceptor.py` (`_build_collect_meta`), `src/openpi/cache/orchestrator.py` (`CheckResult.searched`), `src/openpi/serving/per_step_recorder.py`. Design: [`logs/archive/gate_data_collection_plan.log.md`](../../logs/archive/gate_data_collection_plan.log.md).
...
> **Source**: `src/openpi/cache/interceptor.py` (`hit_executor` / `miss_executor` ctor params), `src/openpi/cache/config.py` (`RoutingConfig` + `_routing_errors` allowlist), `src/openpi/cache/sidecar_executor.py` (`SidecarExecutor`), `scripts/serve_policy.py` (`_build_routing_executors`). Design: [`logs/archive/ablation_study_plan.log.md`](../../logs/archive/ablation_study_plan.log.md).
...
> **Source**: `backends/in_memory_backend.py` (bucket index), `components/search_strategy.py` (`TextIvfKnnStrategy`), `components/key_builder.py` (prompt-pool knobs + `find_instruction_span`), `models/tokenizer.py` (`encode_fragment`), `config.py` (`TextIvfIndexConfig`, binding check, `PROMPT_POOL_KNOB_BUILDERS`), `interceptor.py` (`_canonical_tokenized_prompt`). Design: [`logs/archive/text_ivf_prompt_bucket_plan.log.md`](../../logs/archive/text_ivf_prompt_bucket_plan.log.md).
...
GR00T N1.5 (LIBERO, k=8, measured table P = 13.338, L = 28.104, M = 41.442 ms):
│ Stage1 ││ Stage2 (LLM) ││ cp2_encode ││ CP2 check ││ Stage3: skipped (FULL) /
│ Vision ││ backbone     ││ vlln+attn+ ││ proj+cos  ││ prologue again + 1 step from x_0.875 (WARM) /
│        ││              ││ state enc  ││           ││ upstream get_action, 8 steps (MISS)
FULL = P + E, WARM@0.875 = P + E + L/8 = 16.851 + E, MISS = M + E; E per suite from
`exp/libero_groot/bench_cp2_overhead_groot.py --mode encoder-cost` (certified), the
decision overhead from `--mode overhead` (stage 2 outside the boundary).
```
...
| `cp2_*` | cpu | ✅ Registered | CP2 sub-step probes (`cp2_gate/build/search/judge/sum`), CP2-only configs (see Section 3); on GR00T also `cp2_encode` (the key-source encoder forward, registered by `GrootStagedRunner`) |
