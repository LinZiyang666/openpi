...
现有的 `exp/common/build_in_memory_cache_artifact.py` 和 `exp/common/build_clip_cache_artifact.py` 是面向 Pi0.5 HDF5 schema 的参考脚本。迁移时需参考其逻辑编写你自己的构建脚本。
...
**Artifact 构建**: 使用 `exp/common/build_clip_cache_artifact.py` 作为参考。它从 HDF5 中读取原始图像，通过 CLIP 编码后构建 artifact。
