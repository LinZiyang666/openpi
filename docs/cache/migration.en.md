...
The existing `exp/common/build_in_memory_cache_artifact.py` and `exp/common/build_clip_cache_artifact.py` are reference scripts targeting the Pi0.5 HDF5 schema. When migrating, use their logic as reference to write your own build script.
...
**Artifact building**: Use `exp/common/build_clip_cache_artifact.py` as reference. It reads raw images from HDF5, encodes them via CLIP, and builds the artifact.
