...
Step 1b  run_step1b_gt.py           → GT HDF5（AlwaysSkip，固定 clip_w7_d4 bundle）
...
| Step 1b GT 采集 | **不能并行** | 每个 unit 都是独立 `main.py` 子进程 + libero env；当前 runner 使用串行 `run()` 而非 `parallel_run()` | 强行多开会抢 GPU/显存；同一 bundle 切一次即可（`inference_clip_w7_d4.yaml`） |
...
| `--cache_config` | 启动时的初始 bundle | 任选一个即可，建议填 GT bundle（`inference_clip_w7_d4.yaml`）这样 Step 1b 可以用 `--skip-config-switch` 走更可控路径 |
...
| `--collect-images` | 额外保存原始 image（CLIP KeyBuilder 需要） | 只要 bundle 里用到 `key_builder.type: clip` 就必须开；`deviate_exp/` 的 3 组 YAML 都是 clip key builder，所以总是开 |
...
plan §18.B4.3 把 GT bundle 锁死在 `inference_clip_w7_d4.yaml`——Step 2 的三个配置必须共享同一 GT，否则 deviate_score 的分母失去可比性。Runner 启动时调一次 `send_load_cache_config`，然后所有 unit 串行跑 `main.py`（每个 unit 单独起一个 libero env 子进程）。
...
| `--inference-yaml` | GT bundle YAML 路径 | **不要改**；默认就是 `inference_clip_w7_d4.yaml`，换了就违反 plan §18.B4.3 |
...
| 参数 | 值 | 理由 |
|------|-----|-----|
| GT bundle | `inference_clip_w7_d4.yaml` | plan §18.B4.3：三 cfg 共享同一 GT |
...
| Step 1b 大量 `inference_failed=True` | server 挂了 / 不在 `inference_clip_w7_d4.yaml` | 检查 server 进程；若你加了 `--skip-config-switch`，确认已手动 `load_cache_config` 到 GT bundle |
