...
Below is the complete plan text compiled from the discussion phase and the post-defense revision.
...
| Step 3 files (4) | Modify (add coupling annotations) | — |

### Core Design Decisions

1. **KeyBuilder dual responsibility**: `collect()` holds GPU tensor references (zero-copy), `build()` performs GPU normalize + D2H
2. **Gate/Judge receive `cached_data`**: forward-compatible with future implementations that read KeyBuilder data
3. **Orchestrator interacts only through `CacheStorage` facade**: never touches the underlying backend
4. **Orchestrator holds the timer**: centralized fine-grained timing management, components do not hold timers
5. **Interceptor degrades to zero overhead when `orchestrator=None`**: backward-compatible
6. **Each file includes a Coupling map annotation**: DEPENDS ON / CONSUMED BY / IF CHANGED
7. **Add coupling annotations to Step 3 files**

### Implementation Order

1. `components/__init__.py` → 2. `key_builder.py` → 3. `gate.py` → 4. `judge.py` → 5. `orchestrator.py` → 6. `interceptor.py` modification → 7. `__init__.py` update → 8. Step 3 coupling annotation backfill

---


## Implementation Plan v2 (Post-Defense Revision Full Text)
...
`SystemTimer` provides `register_probe(name, backend)` + `with timer.measure(name)`. When `enabled=False`, `measure()` is a pure no-op (zero overhead).

**Orchestrator holds the timer reference** and times each sub-step. Components (KeyBuilder/Gate/Judge) do NOT hold the timer — timing is managed centrally to avoid spreading timer refs into unstable components.
    ...
    def check(self, checkpoint_id: CheckpointID, **stage_outputs) -> CheckResult:
        ...
        Flow:
          ...
          4. storage.search(
             ...
             )  # [reviewer edit] use keyword args to avoid accidentally passing checkpoint_id as top_k
