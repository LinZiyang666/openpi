...
## 5e. 门后的 IR 地板 —— RIT 的固有边界（owner 裁定 2026-09-11：不处理，这是结果）

RIT 臂第一次跑满后，`realized − addressed` 的漂移是**强单调**的（libero_spatial）：

| target | 20 | 30 | 40 | 50 | 60 | 80 | 95 |
|---|---|---|---|---|---|---|---|
| k=1 实测 | 38.97 | 44.43 | 46.68 | 50.01 | 54.46 | 67.85 | 89.77 |
| 漂移 | **+18.97** | +14.43 | +6.68 | +0.01 | −5.54 | **−12.15** | −5.23 |

两个机制同时作用，可由符号区分：

- **便宜端的正漂移是门的地板。** `score_hysteresis` 的 `L=6` 锁定强制每 ≤6 个缓存步插一次真推理，
  所以约 1/7 的步恒为 MISS，**IR 有一个约 39 的硬地板**。目标 20/25/30 不可达。
  ⚠ `delta_for_ir` 对低于地板的目标**仍会返回一个解**——它在标定分数上求解，不知道门的存在。
  低目标的那几个臂因此全部挤在地板上。
- **昂贵端的负漂移不可能是门**（门只推高 IR），是闭环协变量漂移：部署时落在切点之上的质量
  比标定预测的多。方向与 pi0.5 v1 相反、幅度温和。

**后果**：k=1 的 16 个点预测最小间隔 4.95，**实测压到 0.98**（中位 2.87）；k=2 最小 0.51。
寻址区间 20–95 压缩成实测 39–90。

**owner 裁定：不处理。** 地板以下本来就没有可行工作点，这是 RIT 在门后的能力边界，不是测量缺陷。
前沿按**实测 IR** 画，每个 (IR, SR) 都是真实测量，横轴不依赖寻址是否命中，所以数据全部有效。
若将来要覆盖便宜端，只能去掉门（无门层）或按门后成本重新寻址，两者都要另跑一轮。

## 5f. ⚠ 聚合时必须遵守的一条：libero_10 的锚点只取 timan108

l10 收尾时两条 lane 的分工是：timan107 补最后 2 个 hg 臂（`rit_k1_ir40` / `rit_k1_ir45`，
它们已在它自己的 journal 里跑了一半，迁走会把同一个臂的集数拆到两个 journal），
timan108 空闲下来后接**两个锚点臂**跑完整的 1000 集。

但 timan107 的 journal 里**已经存有 392 集锚点数据**（`anchor_cache` 304 + `anchor_teacher` 88，
是更早一轮被中断留下的）。于是这 392 集在两台机器上各有一份：

- **成功率不受影响** —— 聚合器按 `task_uid` 去重，同一个 uid 只算一次。
- ⚠ **IR 会错** —— per-step 行是逐行累加的，两边各自匹配自己 journal 的 accepted attempt，
  合并后那 392 集的判据被数两遍。

⇒ **`libero_10_anchor` 只从 timan108 聚合**（它有完整的 1000 集），timan107 的锚点分片弃用。
hg 组两边都取（臂集合不相交，无重复）。
