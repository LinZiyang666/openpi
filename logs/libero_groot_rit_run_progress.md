...
## 5d. 早期读数（libero_spatial，13 臂跑满，成本口径为临时值）

| 臂 | IR% | SR |
|---|---|---|
| 纯 cache（全 FULL_HIT） | 17.166 | 0.720 |
| `gst_f00w40v00`（40% 浅档 warm） | 84.060 | **0.946** |
| `gst_f00w40v20` | 79.024 | 0.940 |
| 纯推理（全 MISS） | 100.000 | 0.932 |

⚠ 两个标定点精确命中：全 FULL_HIT = `stage1/miss` = 17.166，全 MISS = 100.000。
⚠ **实测判据份额复现了寻址份额**（误差 1-2 pt：寻址 20/40/60/80% 对应实测 18.5/40.8/60.8/81.7%）
⇒ 闭环分数分布与 warmup 分布没有明显漂移，这是 IR 寻址成立的前提。

**配对比较的口径**：GST 单纯形天然给出同成本、同库、同门、同集的配对，唯一差别是份额怎么放。
⚠ 每臂 500 集 ⇒ 两个 SR 之差的标准误约 **1.7 个百分点**，任何小于这个数的差都不能当结论。
工具在 timan108 的 `/tmp/warm_split.py`（按 ΔIR≤1.5、份额差≥0.2 配对）与 `/tmp/full_share.py`。

| 比较 | 配对数 | 平均效应 | 判定 |
|---|---|---|---|
| warm 放**浅档**（剩 2 步）vs 深档（剩 4 步） | 10 | +0.9 pt | **噪声内，不成立** |
| **FULL_HIT 份额↑**（原样复用，零重新去噪） | 2 | **−4.7 pt** | 每对约 2.8 SE，同向，强提示 |

⚠ 教训：我在只有 2 对时就断言过"浅档更优"（+2.0 / +3.4），样本涨到 10 对后变成 7 胜 3 负、
均值远小于一个标准误。**配对比较必须先算标准误再下结论。**

FULL_HIT 那两对是 IR 68.0/68.7（0.906 vs 0.946）与 IR 72.1/71.3（0.890 vs 0.944）。若成立，
含义是 **cache 的价值来自 warm-start 而非原样复用**：FULL_HIT 便宜到能多买缓存步数，仍换不回
这 4-5 个点。RIT 的 48 个臂按风险混合各档，会更直接地检验它。

这也解释了 IR 分离度检查报的 2 对 collapsed：它们是**跨 cell 的成本碰撞**（warm@0.75 29.27 ms
vs warm@0.5 35.28 ms，所以 40% 浅档 ≈ 20%+ 深档），不是网格塌掉，而且正是上表的信息来源。
真正要防的是**同一条规则内部**塌掉——RIT 每个 k 的 16 个点预测最小间隔 4.95，待实测复核。
