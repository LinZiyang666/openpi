...
**术语统一（2026-08-31 20:50，owner 裁定，全库生效；本条为 append-only 追加，G1 冻结前缀与既有条目不改写）**：GST = Grid-Searched Threshold（网格搜索阈值）——本日志中的 threshold / thr / T / tgrid / (fh, ws) 网格 / 速率索引均指它；RIT = Risk-Indexed Threshold（风险索引阈值）——s-only / s0 / 校准分位切 / 风险阶梯 / surface 均指它，(s,v) 版 SV 为 RIT 的消融。已改写的现行文档：`docs/iclr/latex/sonly_note.tex`（PDF 已重编）、`logs/dispatch_surface_sonly_pivot_report.md`、`logs/session_handoff_dispatch.md`、`docs/architecture/cache_system.md` judge 表、`docs/iclr/README.md`、图例（`plot_budget_amendment` / `plot_suite_frontiers` / `oracle_headroom`，图已重绘）与相关 docstring；历史结果/计划文档只加顶部术语注释；`dispatch_surface_rev2_protocol_draft.md` 因整文件 SHA 冻结（`confirmation_freeze_record.json`）不改。
