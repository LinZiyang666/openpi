...
- **2026-08-22 19:5x → 08-23 08:4x 本地 T5b 库扩容完成（owner 令 20→50 条/任务，先 GR00T 后 pi05）**。两侧各 13/13 达标、id 全致密、零数据损失。
  - **GR00T**：612 主批 + 9 补批 = 621 次尝试，四路（4 collector server 8020-23 + 4 driver，1:1:1）约 3.7h；成功 337→**704**（50-61/任务）；文件 579→1200。
  - **pi05**：750 主批 + 15 补批 = 765 次尝试，同拓扑（8010-13）约 8.9h；成功 382→**704**（50-61/任务）；文件 774→1539。/data 用量 888G→1.4T。
  - **续采机制**：`--episode-lo` 接既有末端，`seed = base_seed + episode_idx` ⇒ id 即种子，新采样与旧集零重叠；批量按二项 0.90 分位定，两侧各只有一个任务需补批（GR00T Tstr→Ctr 实测 p=0.61<历史 0.79；pi05 三任务差 4-6 条）。
  - **教师真实成功率（本批实测，比准入门 n=5 可靠）**：GR00T 0.43-0.82；pi05 0.13-0.91，最难 CloseBlenderLid **p=0.13**（220 次换 52 条，单路耗 9h，占 397 文件），OpenStandMixerHead 0.32。pi05 单集耗时约 GR00T 的 1.8×（10 步去噪 vs 4 步）。
  - **踩坑与守卫（全部按设计 fail-fast，非缺陷）**：① `--collect` 与并发结构性互斥，`serve_policy.py` 必须显式 `--non-concurrent --replicas 1`（并发是默认）；② run-plan 守卫拒绝参数变更后续写旧账本，续批须 `--batch N+1`，且失败尝试会留空壳 plan 占号，重发前先删；③ **端点组守卫**：`collect_weilandserver.env` 的 `PI05_SERVERS` 原只声明 8010/8011，driver 拒服务未声明端口——已扩至四端口；④ **孤儿 worker 事故**：`tmux kill-session` 杀不掉 `start_new_session` 的 worker，两个孤儿霸占 8022/8023 唯一连接位使新 driver 被 **1013** 拒连、重试 30min 后死，lane2/3 空转 1h；处置=按 pgid 定点回收 + 核端口连接归零再发车；⑤ **路径错位**：`--collect-hdf5` 多给一层导致 GR00T 新数据落在 `groot_tp/groot_tp/`，收尾统一 mv 归位（621 集，零 id 冲突）；⑥ 监控脚本两 bug：`tether exec` 默认 HOME 非用户 home 致 `tmux ls` 误判车队全灭、`pgrep` 不带 `-f` 永不匹配 python 脚本。
  - WSL 重启一次（owner 计划内），远端 tmux/tether 全程无感，仅本机 Monitor/cron 按 handoff §0 清单重挂。
  - **下一步待 owner 裁决**：新库口径（沿用统一 n5/n10/n20 ↔ 按物体类别分桶 ↔ 两者都建），见 round1 报告 §5 建议。
- **19:0x-19:15 本地 快速前期实验：胜出 cell 同种子整格重跑（owner 直令）**。`grid_vision_2@87_robot_state@12` 用 round-1 完全相同的 yaml/种子重跑全部 104 集（4 GR00T server × 32 worker 任务切分 4/3/3/3，10min 收官，journal 隔离 /tmp/rr_*，已拉回 scratchpad rr/）。**配对复现矩阵：成功→成功 28/28（100%）；失败→失败 73/76；失败→成功 3（PickPlaceSinkToCounter s2 / SlideDishwasherRack s6 / TurnOnSinkFaucet s6）；成功→失败 0**。macro 第二读数 0.298 vs 0.269。判读：①纯 cache 管线 97% 逐位确定（成功是 (初始状态×配置) 的函数不是抽签），残余 ~3% 翻转仅在失败侧边缘 episode（GPU 浮点非确定性漂移过线）；②cell 间同种子差异因此几乎全为配置的真实函数性差异——132 格的配对检验偏保守，打平集合是下界口径；③加密轮原设的"同 seed 复现控制"就此有答案（~97%/成功侧 100%）。⚠ 执行中把 pkill 自匹配坑踩了第二遍：模式里**每个**字面片段都须括号装甲（"[w]s2rerun" 旁边不能有明文 "ws2rerun" 出现在同 argv 任何位置，包括另一个 pkill 的模式）。机群已清场（显存回 48G）。
