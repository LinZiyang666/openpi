...
> **实验全部完成 2026-09-11 02:00 CDT。** 所有 server/worker/监控已关闭。
...
P1  GR00T 全线（k=2 / k=3 / teacher）    [##########] DONE  8,450 + 650 集
P2  pi0.5 全线（v2 重标定后）            [##########] DONE  9,100 集，IR 成功铺开
P3  四线帕累托图                         [##########] DONE
P4  k=1 双 teacher                       [##########] DONE  GR00T 4,550 + pi0.5 4,550
```

**全部 27,850 集，err / missing 全程为 0。**
...
## 10. k=1 控制族最终结果（13 任务，n=650/arm，err 0）

| arm | GR00T IR | GR00T SR | pi0.5 IR | pi0.5 SR |
|---|---|---|---|---|
| always_hit | 43.72 | 0.4492 | 26.78 | 0.3954 |
| ir 最便宜 | 43.08 | 0.4600 | 26.88 | 0.3662 |
| ↓ | 54.69 | 0.4754 | 43.66 | 0.3862 |
| ↓ | 63.22 | 0.5431 | 52.59 | 0.5031 |
| ↓ | 70.03 | 0.5877 | 66.80 | 0.5477 |
| ↓ | 77.53 | **0.6938** | 85.10 | **0.5754** |
| ir100 | 99.86 | 0.6692 | 99.98 | 0.5662 |

两个 teacher 的 k=1 都在 IR≈78-85 处达到峰值并**超过 IR=100 的全推理臂**（GR00T .694 vs .669；pi0.5 .575 vs .566）。

前沿文件：`~/tmp_rit/frontier_k1_{groot,pi05}_all13.json`。

## 11. 收尾状态（2026-09-11 02:00 CDT）

- **全部关闭**：timan107 / weilandserver worker 归零，h100 server 与传输 daemon 关停，232xx / 231xx 端口全释放，三处 GPU 归 0。
- **数据已拉回本地**：`~/tmp_rit/k1_products/`（28 份 summary + per_step），四条 lane 的前沿 JSON 已生成。
- ⚠ **timan108 仍 OFFLINE**：45 worker 压死 NVIDIA 驱动（2026-09-10），需 BMC 复位；本地盘上困着 970 集作废数据（已在别处重跑，不影响完整性）。
