...
> **Frozen historical outputs.** The migrated `exp/qdrant_step_knn/data/example/{config.json, details.csv, summary.csv}` and `exp/qdrant_step_knn/analysis/example_report.md` are verbatim artifacts of a past run and are deliberately not rewritten. Their internal references to old `data/libero_spatial*` paths are preserved as historical record, not active configuration, and are therefore out of scope for the Phase 7 path sweep. The active user-facing `exp/qdrant_step_knn/config/qdrant_step_knn_experiment_config.example.json` is the canonical input and was updated in Phase 7.
...
### G2 Round 1 — Reviewer — NEEDS REVISION — 2026-04-17 16:59 CDT

- [Blocking] [Concern] Make the documented pytest command work without an out-of-band `PYTHONPATH=.` environment override — reasoning: `uv run pytest tests/exp tests/scripts` fails during collection with `ModuleNotFoundError: No module named 'exp'` for every `tests/exp/test_*.py` import of `exp.*`. The same target set passes only when rerun as `PYTHONPATH=. uv run pytest tests/exp tests/scripts` (208 passed), so the code is likely sound but the repository test entrypoint is not. Phase 9 and Working Agreement §2.7 require `uv run pytest`, not an unstated local environment workaround.
- [Blocking] [Concern] Remove remaining active `configs/cache_runs` output paths from the YAML generator and runbook commands — reasoning: `exp/common/generate_cache_run_yamls.py` still documents and defaults `--output-dir configs/cache_runs`, and `docs/experiments/cp1_cache.md` still instructs users to pass `--output-dir configs/cache_runs` while describing outputs under `exp/common/config/phase1/`. Running the current command will recreate the deleted old tree, contradicting the approved layout and Phase 7 path-sweep requirement.
- [Blocking] [Concern] Route warm-start analysis outputs to `exp/warm_start/analysis/` and make the runbook match the script's actual inputs — reasoning: `exp/warm_start/analyze_warm_sweep.py` writes `success_rate_sweep.png` to `exp/warm_start/data/results/`, while the approved canonical rule puts PNG/PDF/analysis outputs under `analysis/` and the tracked migrated image is `exp/warm_start/analysis/success_rate_sweep.png`. `docs/experiments/warm_start_sweep.md` and `.en.md` also say the analyzer reads `exp/warm_start/data/results/cache_eval_results.json`, but the script reads the per-variant tracked JSONs under `exp/warm_start/data/{max_pool,spatial16,clip}/cache_eval_results.json`.
- [Blocking] [Concern] Finish the stale bare-`data/...` path sweep for active commands/configs — reasoning: active files still point at removed top-level data locations, including `exp/common/calibrate_robot_state_tau.py` defaulting `--data-dir` to `data/libero_spatial`, `exp/common/build_in_memory_cache_artifact.py` usage examples passing `--data-dir data/libero_spatial`, and `exp/qdrant_step_knn/config/qdrant_step_knn_experiment_config.example.json` using `data/libero_spatial` / `data/libero_spatial_1`. The approved scope moved the entire top-level `data/` tree under `exp/`, so these defaults/examples should not send future runs back to old paths.
- [Blocking] [Concern] Sync `docs/README.md` with the new canonical experiment-layout document — reasoning: `docs/experiments/artifact_layout.md` was created and registered in `docs/experiments/README.md` and `WORKING_AGREEMENT.md`, but the top-level `docs/README.md` experiments index does not list it. Working Agreement §4's Index Sync Rule requires the corresponding docs README to be updated in the same change.

### G2 Round 1 — Executor (2026-04-17)

- Accepted — pytest entrypoint: added `pythonpath = ["."]` to `[tool.pytest.ini_options]` in `pyproject.toml`. `uv run pytest tests/exp tests/scripts` now collects all `exp.*` imports without a `PYTHONPATH=.` override (208 passed locally). Full `uv run pytest` still at 623 passed / 13 environmental failures (pre-existing JAX/CUDA/model-load tests in `src/openpi/`, unchanged from the pre-refactor baseline).
- Accepted — `configs/cache_runs` sweep: reworked `exp/common/generate_cache_run_yamls.py` so (a) the three docstring examples emit to `exp/common/config/phase1`, `…/phase1_5`, `…/phase2`, and (b) the `--output-dir` CLI argument is now `required=True` with no default, so the command fails loudly instead of silently recreating the deleted `configs/cache_runs/` tree. Updated `docs/experiments/cp1_cache.md` Steps 3/5a/5b/5c/5d/6/7/8/9/10/11/12 plus the §"实验中断后的恢复" snippet and the final pipeline diagram: every `--output-dir configs/cache_runs`, `--yaml-dir exp/phase*`, and `--state-file exp/phase*/…` is routed to `exp/common/config/<phase>/` and `exp/common/data/<phase>/…`, and the `run_cache_experiments.py` invocations now pass an explicit `--state-path exp/common/data/<phase>/experiment_state.json` so the tracked state file (not a stray one under `config/`) is what gets written.
- Accepted — warm-start analysis routing: `exp/warm_start/analyze_warm_sweep.py` now writes to `exp/warm_start/analysis/` (matches canonical rule and the tracked `success_rate_sweep.png`). Rewrote the Step-3 archive block and Step-4 analyzer description in `docs/experiments/warm_start_sweep.md` and `.en.md` so they match what the script actually does: reads per-keybuilder `exp/warm_start/data/{max_pool,spatial16,clip}/cache_eval_results.json` plus the `exp/trajectory_deviation/data/cache_eval_results_*.json` baselines, drops the fake "merge into `data/results/cache_eval_results.json`" step, and points the output at `analysis/`.
- Accepted — stale bare `data/...` sweep: changed the three files G2 flagged to point at `exp/common/data/libero_spatial[_1]`: `exp/common/calibrate_robot_state_tau.py` default `--data-dir`, `exp/common/build_in_memory_cache_artifact.py` docstring `--data-dir` (both occurrences), and `exp/qdrant_step_knn/config/qdrant_step_knn_experiment_config.example.json` (`query_file`, `db_data_dir`). Also swept two active user-facing docs that steer users to `data/libero_spatial` — `docs/data_collection/guide.md` (example HDF5 path + glob snippet) and `docs/cache/tutorial.md` (`--data-dir` example) — now under `exp/common/data/libero_spatial`. Left as-is: `exp/qdrant_step_knn/analysis/example_report.md` and `exp/qdrant_step_knn/data/example/details.csv` are frozen outputs of a past run (historical artifacts, not active config), and `scripts/serve_policy.py --collect_dir` default (`./data`) is out of Phase 7 scope and user-overridable per data-collection guide.
- Accepted — `docs/README.md` index sync: added `experiments/artifact_layout.md` row to the `[experiments/]` table in `docs/README.md`, completing the §4 Index Sync Rule alongside the existing `docs/experiments/README.md` + `WORKING_AGREEMENT.md §8` entries.

### G2 Round 2 — Reviewer — NEEDS REVISION — 2026-04-17 17:22 CDT

- [Blocking] [Concern] Align `generate_cache_run_yamls.py` output-directory semantics with the updated runbook paths — reasoning: the Round 1 fix changed examples and `docs/experiments/cp1_cache.md` to pass phase-specific output directories such as `--output-dir exp/common/config/phase1`, `exp/common/config/phase1_5`, and `exp/common/config/phase2`, but the implementation still computes `Path(args.output_dir) / "phase1"`, `... / "phase1_5"`, and `... / "phase2"` inside `_generate_phase1`, `_generate_phase1_5`, and `_generate_phase2`. As written, the documented command would create nested paths like `exp/common/config/phase1/phase1/*.yaml`, while the runbook then runs `--yaml-dir exp/common/config/phase1`. Either make `--output-dir` the final phase directory and stop appending the phase name, or document/pass the common parent `exp/common/config` consistently.
- [Non-blocking] [Suggestion] Treat old `data/libero_spatial*` references under `exp/qdrant_step_knn/data/example/` and `exp/qdrant_step_knn/analysis/example_report.md` as historical frozen outputs in the final polish note — reasoning: the active config was fixed, and these remaining hits are archived/example result artifacts rather than executable inputs, but recording that distinction will prevent a future reviewer from reopening the same path-sweep question.

### G2 Round 2 — Executor (2026-04-17)

- Accepted — `generate_cache_run_yamls.py` output-directory semantics: the script is already correct (internally computes `Path(args.output_dir) / "phase1"` / `/ "phase1_5"` / `/ "phase2"` at lines 217/232/250), so the double-nesting was purely a documentation bug introduced in R1. Fixed by reverting the three docstring examples in `exp/common/generate_cache_run_yamls.py` back to the parent directory (`--output-dir exp/common/config`), adding `(written to <output-dir>/phase1/)` / `(-> <output-dir>/phase1_5/)` / `(-> <output-dir>/phase2/)` comments so the append semantics is visible at the example site, and rewriting the `--output-dir` CLI `help` text to state: "Parent directory for generated YAMLs. The script appends a per-phase subdirectory (phase1/, phase1_5/, phase2/). Example: --output-dir exp/common/config writes Phase 1 YAMLs to exp/common/config/phase1/." In `docs/experiments/cp1_cache.md`, Step 3 (Phase 1 generate), Step 7 (Phase 1.5 generate), and Step 10 (Phase 2 generate) now all pass `--output-dir exp/common/config`, with an explanatory note under Step 3 (`> --output-dir 是父目录；脚本按 --phase 自动追加 phase1/ / phase1_5/ / phase2/。`) and the Step 7/10 product descriptions call out that the `phase1_5/` / `phase2/` subdirectory is appended by the script. Result: the runbook `--yaml-dir exp/common/config/phase1` / `.../phase1_5` / `.../phase2` paths in the downstream `run_cache_experiments.py` / `serve_policy.py` commands now match what the generator actually writes.
- Accepted — qdrant_step_knn frozen-artifact distinction: added a "Frozen historical outputs" note to §3.4.5 explicitly stating that `exp/qdrant_step_knn/data/example/{config.json,details.csv,summary.csv}` and `exp/qdrant_step_knn/analysis/example_report.md` are verbatim artifacts of a past run, their old `data/libero_spatial*` references are preserved as historical record (not active configuration), and they are therefore out of scope for the Phase 7 path sweep; the canonical active input is `exp/qdrant_step_knn/config/qdrant_step_knn_experiment_config.example.json`, which was fixed in R1. This matches and formalizes the ad-hoc carve-out previously only recorded in the R1 executor response.

### G2 Round 3 — Reviewer — NEEDS REVISION — 2026-04-17 17:32 CDT

- [Blocking] [Concern] Make `docs/experiments/cp1_cache.md` Step 6 write the Phase 1 analysis to the same path that Step 7 consumes — reasoning: Step 6 currently runs `--output exp/common/config/phase1/analysis.json` and tells users to inspect `exp/common/config/phase1/analysis.json`, but Step 7 passes `--phase1-analysis exp/common/data/phase1/analysis.json`, and the generator docstring also documents `exp/common/data/phase1/analysis.json`. As written, the documented workflow cannot proceed from Phase 1 analysis to Phase 1.5 generation without a manual path correction. The analysis result is run data, not generated YAML config, so the consistent path should be under `exp/common/data/phase1/`.
- [Non-blocking] [Note] The Round 2 output-directory concern is resolved: `--output-dir exp/common/config` now matches `generate_cache_run_yamls.py` appending `phase1/`, `phase1_5/`, or `phase2/` internally.
- [Non-blocking] [Verification] `uv run pytest tests/exp tests/scripts` passed: 208 passed, 1 warning.

### G2 Round 3 — Executor (2026-04-17)

- Accepted — Phase 1 analysis path alignment: `docs/experiments/cp1_cache.md` Step 6 now writes `--output exp/common/data/phase1/analysis.json` (matching Step 7's `--phase1-analysis exp/common/data/phase1/analysis.json` consumer and the generator docstring). Also fixed the follow-up verification sentence ("检查 `exp/common/data/phase1/analysis.json` 中的 `top3` 字段") with a cross-reference note that Step 7 consumes the same file, and updated the bottom-of-doc pipeline diagram so the `analyze_cache_results.py` box now shows `exp/common/data/phase1/analysis.json` (consistent with the already-correct `phase1_5/` and `phase2/` entries below it). Additionally fixed the `exp/common/analyze_cache_results.py` docstring usage example, which carried the same `--output exp/common/config/phase1/analysis.json` typo — changed to `exp/common/data/phase1/analysis.json` so the script's own documentation agrees with the runbook. Rationale (project-specific): analysis JSON is a computed result file produced from `experiment_state.json`, so the canonical location is under `data/phase*/` alongside its input, not under `config/phase*/` (which holds generated YAML configs and their run logs only).
- Accepted — Round 2 output-directory resolution note (reviewer observation, no action needed): agreed; no change required.
- Accepted — pytest verification note (reviewer observation, no action needed): agreed; the 208-passed result matches the local run recorded in the R1 executor response, confirming the test entrypoint fix remains stable.

### G2 Round 4 — Reviewer — APPROVED — 2026-04-17 17:36 CDT

- No blocking concerns found. The Round 3 Phase 1 analysis path concern is resolved: `docs/experiments/cp1_cache.md` Step 6 now writes `exp/common/data/phase1/analysis.json`, Step 7 consumes the same path, the pipeline diagram matches, and `exp/common/analyze_cache_results.py` documents the same output path.
- Path sweep spot-check passed for active code/docs: no remaining active `configs/cache_runs`, phase-specific `--output-dir exp/common/config/phase*`, or `exp/common/config/phase1/analysis.json` references outside historical review-log/plan text.
- No tracked deletions are present in the staged diff or working tree.
- Verification passed: `uv run pytest tests/exp tests/scripts` reported 208 passed, 1 warning; `git diff --check` reported no whitespace errors.
