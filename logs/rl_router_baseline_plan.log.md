...
- **标准化两阶段（消循环依赖，R2 修正；v1→v2 见下方 owner 修订）**：**encoder v0-raw** = 无标准化直拼（warm-start 采集期使用，dump 落 raw 特征）；采集后拟合 μ/σ → **encoder v2** = v0 + **整个拼接向量上的一次仿射**（vision 逐字段标量、robot_state 逐维，且把 1/√D 折进 σ）。v0→v2 为确定性变换（μ/σ 存权重 meta，四方共享）。
  > **Owner 修订（2026-08-15，运行期实测驱动）**：原文为「encoder v1 = v0 + robot_state 仿射（vision 字段 builder 已定标不再动）」。括号里那句前提**被实测证伪**——生产 `cp1_spatial_pool_16` 的 vision 字段实测 std≈5.25、range ±209，并未定标。在 D=65,568 这个宽度上后果是致命而非不整洁：Adam 对每参数单步位移≈lr，故单步预激活变化≈`lr·Σ|x_j|`≈82（按 §3.5 冻结的 lr=3e-4），实测**一步就把策略饱和成单臂**（student logit 0.836→−45.49，逐步 probs 塌成单一取值）；此前另一次以 raw 特征拟合的头则是 **0/256 存活单元**、REINFORCE 对第一层的梯度恒为零。两次都让 router 变成与观测无关的常数策略，而进度/join/拒收/成功率等常规信号全程正常。owner 于 2026-08-15 裁定采「encoder 内逐字段标准化 ÷ √D」一族，理由是它**保住 §3.5 的全部冻结常数**（lr=3e-4 / β=0.01 / clip=1.0），改的是被证伪的前提本身。实测验收：同一次 v0→v1 更新由 −46 的悬崖变成 +0.053 的轻推，存活单元 134/256 不变，逐步 probs distinct 413。`encoder_version` 随归一化改变而改变——本就是它的设计意图，故新旧分片天然不会混用。**本条修订尚未经 G2 review**（详见 `rl_router_operations.log.md` §3.12）。
**算子次序与 parity（R4 冻结）**：RL 期在线 encoder = `raw fp32 → robot_state 仿射 normalize → Q(fp32→fp16→fp32)`，**MLP 决策就在 Q 输出上进行**；**RL 期 dump = Q 输出的 fp16 字节**（= MLP 输入的无损表示）→ on-policy parity 按构造精确。**采集期（v0，constant_arm 模式）dump = Q(raw)**（无 normalize——μ/σ 尚不存在）；warm-start 拟合消费 `Q(normalize(Q(raw)))`，与在线 v1 相差一次双重量化——**仅影响初始化质量、不承担 parity 契约**（预注册披露）。dtype/rounding/算子次序全部纳入 encoder_version。**核验设备冻结**：behavior-logprob 核验 = trainer 侧同一 CPU reference（fp32、`torch.set_num_threads(1)`）重算，与 dump 的 logits/logprob **逐位相等（`==`，无容差）**；parity golden 以真实 65,568 维、真实权重断言；trainer 直接消费 dump 特征、绝不重编码。`encoder_version = sha256(fields+顺序+μσ)` 入权重 meta 与每条 dump 行。
