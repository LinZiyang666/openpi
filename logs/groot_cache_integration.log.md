...
⚠ 端口 **8000** 是**他人**的 `serve_policy.py`（7764 MiB）—— **绝不可关**。另有两个他人的 `sidecar_server.py`（3392 + 2772 MiB）。我方：pi0.5 用 8010、GR00T 用 8020。
⚠ **禁止宽模式 `pkill`**，只按自己的 tmux 名操作；`cssrv` / `cscol` / `rlr*` 属别的 session。
...
**(c) inference tensor 不可跨 step 存活并被就地改写。** cache payload 里的 `action_pred` 造于 `torch.inference_mode()` 内（`policy.py:190`）；存进 `CacheStorage` 后，任何在 inference_mode 之外对它的 in-place 操作会 `RuntimeError: Inplace update to inference tensor outside InferenceMode`。⇒ `buffer_for_write` 之前必须 `.float().cpu()` 造新张量。
...
⇒ **「两条路逐位相等」由构造决定就不可能达成**，原稿把它定成"最硬判据"是错的，第一次跑必红、而根因不是掩码逻辑。
...
⚠ 更正：原稿说「`check_forward_kwargs`（`:173-178`）说明上游对该签名有断言」**说过头** —— 它只 `assert not any(k.kind == VAR_KEYWORD ...)`（**仅禁 `**kwargs`，不钉参数名/顺序**），且在 `__init__`（`:171`）执行，事后 monkeypatch 根本不会触发它。不 monkeypatch 的理由是全局副作用与不可审计，不是这条断言。
...
⚠ 更正：原稿说「`interceptor.py` 里 `stage3` 只出现三处」**是低估** —— `grep -n stage3` 约 30 行，还有 `stage3_warm` probe（`:32/:290/:940`）、`run_stage3_from`（`:22/:254-262/:645-663/:939`）、`_compiled_stage3_fn`（`:312/:330-340`）、coordinator 路由（`:237-256`）、WARM_START 分支（`:917-944`）。这**反而加强** D1/D3：pi0.5 的三阶段假设渗透得比原以为的深，更不该硬塞给 GR00T。
...
理由见 §2.7(a)：`vlln` 在 autocast 下走 fp32、否则走 bf16，`max|Δ| = 0.0137`。在线路径、采集器、测试三处只要有一处漏开，G0-D 就会挂，而且失败原因不可见。把 context 收进 runner + 入口断言，是唯一能让"漏开"变成**立刻报错**而非静默错值的做法。
...
class GrootStagedRunner:
    ...
    def __init__(self, model, *, verify_upstream: bool = True): ...
    @contextmanager
...
⚠ **`GrootStage1Output` 不提供 `.to(device)`**。原稿有这个方法，但任何 `.contiguous()` / `.clone()` / `.to()` 都可能让 flash-attn 与 cuBLAS 选到不同 kernel ⇒ 归约顺序变 ⇒ Δ != 0。stage1→stage2 之间**只允许传原对象**。
...
⚠ **`collect()` 也必须覆写**（原稿说"只需抽 `_slice()` 2 行"是错的）：`_CP1BaseKeyBuilder.collect`（`components/key_builder.py:238-243`）读的是 `s1.state` 与 **`s1.prefix_embs`**，而 `GrootStage1Output` 没有 `prefix_embs`，且 `_slice` 还需要 `image_token_mask` / `state_mask` 进 `self._cache`。
...
⚠ **现有测试并不足以充当该重构的非回归证据**（原稿说"现有测试即为非回归证据"过头了）：`tests/cache/components/test_key_builder_cp1_experiment.py` 只断 shape / device / dtype / enabled-field 过滤 / CP2 raise，唯一穿过 `build()` 的数值断言是 `robot_state` 透传（`:134-143`）；池化函数的数值断言是**独立函数**测的，不经 `build()`。⇒ **偏移切错时 shape 不变、测试全过**。M2 必须新增一条穿过 `build()` 的数值金标准。
...
3. try:
     with runner.session():                                    # D10
       3a. stage1 = runner.run_stage1(...)                     probe stage1_vision
       3b. if orchestrator: cp1 = check(CP1, stage1=stage1)    probe cp1_sum
           - FULL_HIT: chunk = cp1.payload.action_chunk        # [16,32] 无 batch 维
           - MISS:     chunk = runner.run_stage2(stage1).action_pred   # [1,16,32]
                       probe stage2_llm / stage2_action
           # 无 WARM_START 分支 —— 加载期已挡死（M4）。运行期 raise 是死代码，
           # 因为 orchestrator.py:672-695 会在 intermediates 为空时静默降级成 MISS。
       3c. # 统一到 pi0.5 的 buffer 契约：[AH, AD]、CPU、fp32、contiguous
           action_cpu = (chunk[0] if chunk.dim() == 3 else chunk) \
                          .detach().cpu().float().contiguous()   # 对齐 interceptor.py:1007
...
4. unnormalized = policy.unapply_transforms({"action": action_cpu[None, ...]})
5. 若非 batch 则 squeeze；附 __hit_meta__
```

⚠ **`action_cpu` 的三步归一化不可省**（原稿直传 `stage2.action_pred`，三处都会坏）：
...
2. **加载期守卫**（一次性拒绝，不留运行期惊喜）：`checkpoints` 集合恰为 `{cp1}`；`judge.type ∈ {threshold, always_hit}` 且 `warm_tiers` 为空；`gate.type == always_search`；`write_policy.type == "never"`（D8）；artifact 的 `key_builder_type` 以 `cp1_groot_` 开头（R10）。
...
5. 新增 `exp/robocasa365/config/groot_cache_cp1.yaml`（冒烟用；`always_hit` judge，见 G0-E）。
...
- `exp/robocasa365/groot_cache_collector.py`：`_ActionPolicy` 包装器，逐步跑 stage1（+stage2），用 `slice_groot_cp1_fields(enabled=None)` 取未降维原始字段落盘。
  ...
  ⚠⚠ **还必须写两个文件级 attrs**，原稿的 schema 清单漏了，漏了会**静默丢数据**：
...
2. **新增 per-step 命中记录器**：把 `__hit_meta__` 落成 JSONL。参照 `examples/libero/episode_runner.py:71-88`（`_hit_row`）+ `examples/libero/main.py:327-328`。⚠ 原稿把 M6 说成"加法式小改"，实际 GR00T client **没有任何等价物**；不写这一层，G0-E 的判据无数据源、后续跑批的 per-step JSONL 也不存在。
...
### M8 —— 阈值标定机制核验（只验机制，不产正式值）

`exp/common/calibrate_robot_state_tau.py` 直接吃 HDF5（h5py + numpy，模型无关）⇒ 对 GR00T 的 HDF5 可直接跑。本单元只验证它能吃 GR00T 数据并产出非退化的 tau；**正式标定值属跑批 plan**。

⚠ 这一条存在的理由：现有 yaml 里的 `tau: 0.334717` 是 **pi0.5 prefix_embs 尺度下标定的**。GR00T tap 点是 Qwen3 输入嵌入，L2 量纲不同 ⇒ 直接复用会让 hit rate 稳定停在 0% 或 100%。这正是 G0-E 改用 `always_hit` 判据的原因（§8）。
...
| T-7 | `tests/robocasa365/test_groot_cache_collector.py` | 非 manual | HDF5 schema、dtype、step 命名、原子写 |
...
| R6 | inference tensor 跨 step 存活被就地改写 | `RuntimeError: Inplace update to inference tensor` | §2.7(c)：`buffer_for_write` 前 `.float().cpu()` |
...
| R10 | **pi0.5 库被 GR00T server 静默加载**（`load_artifact` 只校验 `vector_dims`，两者 vision 都是 32768） | 语义完全不同却不报错 | artifact 写 GR00T 的 `key_builder_type`；GR00T server 加载期断言其以 `cp1_groot_` 开头 |
| R11 | 共享 4090 显存波动 | 起服务撞 OOM 伤到别人 | 连续 3 次、间隔 5 min、均 ≥20 GB 才动手；**端口 8000 与两个 `sidecar_server.py` 绝不可碰** |
...
| 占用方（**都不能碰**） | 8000 `serve_policy.py` 7 764 MiB；两个 `sidecar_server.py` 3 392 + 2 772 MiB | `--query-compute-apps` + `ps` |
...
| **G0-D2 端到端（容差）** | 真实 bf16 → fp16 落盘 → fp32 池化 vs 在线 fp32 池化：相对误差 **< 1e-3**，且**远小于**同一 episode 内相邻步之间的 key 距离 | 停；量化损失已大到会改变检索排序 |
| **G0-E 闭环冒烟** | judge = **`always_hit`**（不依赖任何标定值）：`stage2_llm` / `stage2_action` probe **零采样**、动作确实来自库、无异常、`__hit_meta__` 经 adapter 上线并落进 client JSONL | 定位后重跑 |

⚠ **G0-E 用 `always_hit` 而非 "hit rate > 0"**：现有 yaml 的 `tau: 0.334717` 是 pi0.5 prefix_embs 尺度标定的，GR00T tap 点量纲不同，hit rate 会稳定停在 0% 或 100% ⇒ 该判据既可能假阴也可能假阳。`always_hit` 把"接线通不通"与"阈值准不准"**解耦**，正是 `migration.md:792` 的建议。阈值本身由 M8 的机制核验交给跑批 plan。

⚠⚠ **G0-C 与 G0-D1 是判据不是仪式**：只要不是逐位相等，就必须停下来查根因，**不许用容差糊过去**——本线全部结论都建立在"两臂用的是同一个 teacher"上。
⚠ **G0-D2 是唯一允许带容差的门**，且理由是可陈述的物理事实（bf16→fp16 存储往返），不是"差不多就行"。原稿把 G0-D 整体定成"逐位相等"是错的：§2.9 证明那由构造决定就达不到，第一次跑必红而根因不是掩码逻辑。
...
| **P4** | 未提交文件（handoff §8 五个 + 本计划新增） | owner 未指示提交，按约定不擅自 `git add` |
