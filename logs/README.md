...
| [tracer_phase1_dynamic_depth.log.md](tracer_phase1_dynamic_depth.log.md) | `Implemented` (G1/G2 APPROVED + §6 Verify green 2026-07-07; 待归档) | **L2**: TRACER roadmap Phase 1 — **M3 动态链深 SearchStrategy**。新增 `DynamicDepthKnnStrategy`(套现有 `SearchStrategy` Protocol)逐步选轨迹深度 `T_t`(提案 Eq 22-23),复用 rrf/score_sum 融合;可插拔 `DepthPolicy`(`ConstantDepthPolicy` 非回归默认 + 免训练 `HeuristicDepthPolicy`,特征仅 step/history_len/action_smoothness,`s⁻`/`Δ⁺` 押后)。**零框架/零 verdict/零训练**;constant 策略逐值等价现有固定深度策略(golden 非回归)。config additive(`_valid_strategy_types`+1、`SearchStrategyConfig`+3 可选字段、`_build_search_strategy`+分支、校验);复用 `trajectory_depth/weights` 作 max_depth/max_weights,**截断用未归一前缀权重(Phase 1 不做 renorm,保 constant 全 history 长度非回归;Eq 7 的 Σ=1 押后)**。改动锚点全亲验(search_strategy.py/config.py file:line)。子实验 `exp/zixuan_proposal/`(源提案 PDF 已归此)。上位依据 [`tracer_retrieval_refinement_roadmap`](tracer_retrieval_refinement_roadmap.log.md) |
