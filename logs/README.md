...
|------|--------|-------------|
| [trajectory_deviation_experiment_plan.log.md](trajectory_deviation_experiment_plan.log.md) | `Plan` | Trajectory deviation corrective experiment: 3-phase (offline diagnosis → signal analysis → Oracle correction) |
| [trajectory_deviation_corrective_experiment.log.md](trajectory_deviation_corrective_experiment.log.md) | `Plan` | Detailed corrective experiment plan: GT collection → Deviate Score → Spawn correction, with trajectory-depth prefill scheme |
| [trajectory_deviation_corrective_implementation.log.md](trajectory_deviation_corrective_implementation.log.md) | `In Progress` | Code-level implementation tracking for the corrective work (referenced as a normative footnote from multiple places in `src/openpi/cache/`) |
| [trajectory_deviation_corrective_implementation_review.log.md](trajectory_deviation_corrective_implementation_review.log.md) | `Plan/G2 Review/Verify` | Code review record: Layer A+D+E, Layer B, Layer C, Layer F APPROVED; Verify offline portion landed, end-to-end dry run pending |
