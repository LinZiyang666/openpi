...
| [verdict_phase5_run_commands.log.md](verdict_phase5_run_commands.log.md) | `Plan` | L1: Phase 5 systematic sweep 6-server 执行命令清单 — 240 cell × 100ep × 1 seed 跨 S1/S2/S3 (timan107 frp 8998/8999/9000) + S4/S5/S6 (直连 8001/8002/8003)，per-server cell 数 4:4:4:3:3:3 = 48/48/45/33/33/33；§0 git pull + sanity；§1 emit-warmup-yamls (148 yaml) → run-warmup → emit-eval-yamls (240 yaml)；§4 dump 6 per-server cell-id .txt files；§5 6 batch run-eval (per-server `--cell-ids $(cat S<N>.txt)`)；§6.1 cat 合并 6 batch → 240 行；§6.2 跑 `_dump_decision_gate_table_phase5` 写 5 个 g{1..5}_decision.json；§6.4 plot_pareto + plot_heatmaps；§6.5 打包下载；§9 16 步节点 checklist |
| [verdict_phase5_systematic_sweep.log.md](verdict_phase5_systematic_sweep.log.md) | `In Progress` (G1 APPROVED R3 / G2 APPROVED R2 — 2026-05-09; §6 Verify 334/334 pass; cmd book 完成；待 commit + push) | L2: Phase 5 online 因子系统性探索 — 5 group × 48 cell × 100ep × 1 seed = 240 cell, 6 server 4:4:4:3:3:3 split (S1=48 / S2=48 / S3=45 / S4=33 / S5=33 / S6=33)。G1 单窗口 (6×2×2×2)、G2 多窗口组合 (12×2×2 p1-only)、G3 online 权重 R3 拓展 12 patterns (×2 channel ×2 base)、G4 多因子子集 (12×2×2)、G5 (FH,WS) 4×4 grid × {p1, p2, g6}。**无 src/ 改动** — 复用 phase4 的 `WeightedSumZeroNanComposer` 真加权和 + `reconstruct_scores(..., composer_weights=...)` + `derive_thresholds(...)`，**不**调 `solve_recipe`（后者 hardcode 等权重）。base offline 与 base_recipe 强绑定（p1→g1/state W-FUT, p2→g10/action W-FUT），G5 不注入 base offline。100ep 噪声决议规则降级：G1-G4 winner-only-if-Δ≥5pp 否则 inconclusive；G5 走 Pareto frontier，不选 winner。`phase5/runner._run_one_cell_phase5` 是 phase4 helper 的兄弟（mirror 结构 / 不复用，避 RECIPES_PHASE4 / phase4 warmup 路径耦合）。warmup 复用：G5 完全复用 phase3 g6 + phase4 p1/p2 历史 jsonl（0 新 warmup）；G1/G2/G4 各 cell 独立 warmup ~148 yaml；G3 共享 4 warmup。data 落 `data/phase5_systematic/` 避免与 stage5 `data/phase5/` 冲突。预算 ~6.5h wall-clock（2.5h warmup + 4h eval）|
