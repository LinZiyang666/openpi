...
| [cache_latency_bench_depth_study.log.md](cache_latency_bench_depth_study.log.md) | `In Progress` (Code 完成 / 待 G2, 2026-05-31) | **L2 研究结果**: CP1 六段延迟 ~ trajectory_depth(1/3/4/5) 受控扫描 50ep/2640step。延迟集中 search 段(~37ms = 95% total)；**H1 证伪** — search 不随 depth 单调升(36-39ms 无趋势)，因 score-memo 跨步复用全库 cosine 摊销 depth 边际成本(T3)；验收逆序经 d4 重跑(search median 39→43ms，run-to-run 噪声 > 跨 depth 差)诊断为噪声非失败；附带 MISS 随 depth 线性增(d1/d3/d4/d5 = 0/50/100/150，语义效应非延迟)。零改 src/infra |
| [cache_latency_bench_search_optimization_report.log.md](cache_latency_bench_search_optimization_report.log.md) | `Complete` (R1/R2/R3 完成, 2026-05-31) | **研究/优化报告**: cp1_search(~37ms=95% CP1) 压到 <10ms/<5ms 的 **3 轮专家 agent** 研究(R1 探索6 + R2 实测5 + R3 对抗验证4 + 综合/报告)。结论：**fp32 预归一化双 GEMV CPU fast path**(load 时建 per-(task,field) L2-预归一化连续矩阵 → 每 query 两次 `torch.mv` 替换 `_compute_field_scores` 的 `torch.stack`+`F.cosine_similarity`)，实测 **median 2.46ms / p99 ~5ms idle(~35-40× 加速)**；**3-way verdict 等价 0/2640**(含真 backend live-parity 0/2640/2640 + row-order 错位验证)，fp16/bf16 翻转 15/194 → **fp32 only**；GPU 仅独占节点值得(共卡 30% 占空比 p99 20.5ms)。残余风险：knife-edge margin(1.83e-7 < fp 误差 1.07e-4) + 新增线程非确定性(7.08e-5) → 可选 top-K fp32 rescore；**带宽争用破核绑定**(pin4+带宽 co-tenant p99 17.7ms) → 生产 rebench gate(§10)。零改 src/(纯研究) |
| [cache_latency_bench_depth_study_rrf_kin.log.md](cache_latency_bench_depth_study_rrf_kin.log.md) | `Complete` (2026-05-31) | **派生延迟扫描**: CP1 六段 ~ trajectory_depth(1/3/4/5) 受控扫描，但换 **weighted_rrf + kinematic composite judge**(phase5 g5_g6 基底 + offline 标定) on **libero_spatial** 50ep/1062step。结论：search 仍占 ~88-91%；**出现 score_sum 扫描没有的 d1→trajectory 台阶**(d1 单步 12.3ms vs d3-5 轨迹 15.5-16.3ms，+25%，T1 单步/轨迹两函数显化)，但 **d3→d5 仍无趋势**(score-memo T3 摊销复现)；judge verdict 深度稳定(kinematic 按动作因子打分非检索 top_score)；composite judge 段升至 ~0.45ms(vs threshold ~0.011ms)。零改 src/infra |
| [cache_latency_bench_round1_stack_elim.log.md](cache_latency_bench_round1_stack_elim.log.md) | `In Progress` (Round 1 完成 / 进 Round 2, 2026-05-31) | **cp1_search 渐进调优 Round 1**（Owner 独裁令工作流 override G1/G2，本会话内审查）：第一刀 = `PrebuiltMatrixBackend(InMemoryBackend)` 只 override `_compute_field_scores` —— 预建 per-(ckpt,task,field) 连续 fp32 矩阵 + id→row，整桶有序时**零拷贝**直接用常驻矩阵、否则 `index_select` gather，**保留原 `F.cosine_similarity`/`torch.norm` 算子** → bit 等价。对比法 LOO(index_select)+self-included(zero-copy) 各 2640 真实 query `verdict_flips=0 / winner=0 / diff=0.0`；3 审查 agent 全 APPROVE。latency 33.9→**21ms(1.61×)**。micro-bench 定位 `F.cosine_similarity` 比纯点积慢 **24×**、prenorm-dot 同桶 **2.76ms** → Round 2 目标。零改 src 框架 |
| [cache_latency_bench_round2_prenorm_dot.log.md](cache_latency_bench_round2_prenorm_dot.log.md) | `In Progress` (Round 2 完成 / **目标达成** / 进 Round 3, 2026-05-31) | **cp1_search 渐进调优 Round 2 = prenorm-dot**：`PrenormDotBackend(PrebuiltMatrixBackend)` prebuild 时 L2 归一化 cosine 桶 → search 时 `mat.mv(qn)` 单次 fp32 GEMV 消除 `F.cosine_similarity`（Round1 micro-bench 实测其比纯点积慢 **24×**）。**cp1_search median 21→4.70ms（✅ <5ms）/ p95 53.76→7.66ms（✅ <10ms），vs baseline 7.2×/12.7×**。非 bit 等价（改数值路径）但 near-threshold subset(122) × {LOO,zero-copy} × threads{1,4,8} 全 `verdict_flips=0/winner=0`，**`geometric_safety_max=-1.83e-7<0` 数学证明 0 翻档**；fp32-ONLY；rescore 杠杆 deferred。2 审查 agent APPROVE。零改 src 框架 |
| [cache_latency_bench_round3_lean.log.md](cache_latency_bench_round3_lean.log.md) | `In Progress` (Round 3 完成 / 进 Round 4, 2026-05-31) | **cp1_search 渐进调优 Round 3 = LEAN（框架开销消除）**：实测推翻草拟的"latency 触底"——roofline 纯算子 2.55ms vs ReplayHarness search 4.70ms，差 ~2ms 是 backend 框架冗余。`LeanSearchBackend(PrenormDotBackend)` override `_search_weighted_score_sum`：稳态直接 GEMV→normalize→加权和→topk(1)，跳 `_batch_field_scores` wrapper / 整桶恒等 scatter / dispatch；`_lean_bucket` 首次 O(n) 验证整桶有序 + 缓存 + O(1) 端点检查。**search median 4.7→3.7ms / p95 7.66→5.56ms，total p95 9.74→7.66ms**（贴边→余量）。等价 self-included 122 subset 0 翻档 + 单测 vs Round-2 bit-identical。审查 APPROVE（响应 MAJOR 端点检查 + MINOR topk/回归测试），6 单测。零改 src |
| [cache_latency_bench_round4_build.log.md](cache_latency_bench_round4_build.log.md) | `In Progress` (Round 4 完成 / 进 Round 5, 2026-05-31) | **cp1 调优 Round 4 = build 优化（batched avg_pool2d keybuilder）**：`CP1SpatialPool16BatchedKeyBuilder` 把 src 双 `adaptive_avg_pool2d` 换成 2 camera stack 一次 `avg_pool2d(k=4,s=4)`+一次 D2H+split。**build median 1.28→0.45ms（2.85×）、total 5.22→4.15ms**。等价最强：`query_key 逐 bit 一致(max_diff=0.0)`+verdict 0+hit_rate 1.0。审查 APPROVE（BIT EQUAL/SHIP CLEAN/SRC UNTOUCHED YES）。诚实标注 lever3(非连续视图快路径)是 CPU artifact，生产收益(lever1 fixed avgpool + lever2 合并 D2H)量级更小但 portable+双设备 bit-exact。只 ship batched，matmul/per-field 探索 untracked 不入库。零改 src |
| [cache_latency_bench_round5_mem_release.log.md](cache_latency_bench_round5_mem_release.log.md) | `In Progress` (Round 5 完成 / 进 Round 6, 2026-05-31) | **cp1 调优 Round 5 = 内存释放（ReleaseVisionBackend）**：快路径只读预建 `_mat`、从不读 entry vision 张量(是 `_mat` 字节重复)，故 `release_vision()` 把 entry 的 cosine+disabled vision 字段换成共享 `torch.empty(0)` 哨兵(保留 key、快路径不变)，配 fail-closed 守卫(释放后 fallback raise `ReleaseUnsafeError`)。**释放 1059.7MB**(692 active 重复+368 dead vision_2/prompt_emb)，latency 不回退(total 4.46ms)，hit_rate 1.0(检索不变)。审查 APPROVE(GUARD COMPLETE/SRC UNTOUCHED YES，穷举所有读 entry 入口无绕过)，5 单测。零改 src |
| [cache_latency_bench_tuning_final_report.log.md](cache_latency_bench_tuning_final_report.log.md) | `In Progress` (6 轮调优收官, 2026-05-31) | **cp1 latency 渐进调优最终总报告（5 轮 + 评估）**：cp1_total **35.49→4.15ms(8.5×)** + 释放 **1059.7MB**，全程零改 src + 每轮独立审查 APPROVE + 0 verdict 翻档。逐轮 R1 stack消除/零拷贝→R2 prenorm-dot(✅达标)→R3 LEAN→R4 build→R5 内存释放。**Tier 2(fp16 实测 1.56× 但翻档 3.24e-4 需 rescore+keep_raw 与释放冲突)/Tier 3(降维) 评估否决**。生产落地建议(src 路径+§10 rebench+GPU bit-exact 复验)+风险(WSL 偏差/带宽争用)。算子层已榨干(search 87% 逼近带宽底) |
| [cache_latency_bench_rrf_round1.log.md](cache_latency_bench_rrf_round1.log.md) | `In Progress` (RRF Round 1 完成 / 进 Round 2, 2026-06-01) | **weighted_rrf 渐进调优 Round 1 = 嫁接验证**：亲验 weighted_rrf 与 weighted_sum **共享同一热核** `_compute_field_scores`(_search_weighted_rrf:530→_batch_field_scores(sid=qid=None)→_compute_field_scores)，故 ship 的 PrebuiltMatrix(R1)/PrenormDot(R2) backend 对 RRF **自动生效零新代码**。新增 libero_10 RRF config + compare_rrf_equivalence.py(**winner-id parity 判据**——RRF argsort 离散 tie 使几何安全界失效) + run_rrf_latency.py + 单测。等价 R1 bit-equal(rrf_score diff 0.0)/R2 winner parity(0/534 翻,仅 2 个 score 微动 1.65e-5)。**latency RRF search 30.87→4.73ms(6.5×)，与 weighted_sum 同库 4.70ms 持平**(印证 owner 'RRF 不差于 sum')。审查 APPROVE(REUSE VALID/RRF EQUIV SOUND/SRC UNTOUCHED YES，0 finding)，3 单测。零改 src |
| [cache_latency_bench_rrf_round2.log.md](cache_latency_bench_rrf_round2.log.md) | `In Progress` (RRF Round 2 完成 / 进 Round 3, 2026-06-01) | **weighted_rrf 调优 Round 2 = prenorm 等价深挖 + fp32-ONLY 定档**（零新 backend）：**dtype sweep 证 RRF 对降精度远比 weighted_sum 敏感**——fp16 翻 RRF winner **187/2640(7.08%)** vs sum 0.57%(12×)、bf16 **809/2640(30.6%)** vs sum 7.3%(4×)，因 argsort 排名对小误差 swap rank（vs sum 的 normalize 平移保 argmax）→ **fp32-ONLY 对 RRF 更硬**。多线程巩固：prenorm winner parity threads{1,4,8}×{LOO,zero-copy} 全 0-flip（非 schedule artifact）。审查 APPROVE(RRF FUSION FAITHFUL/FP32-ONLY JUSTIFIED/SRC UNTOUCHED YES，独立 probe 0/2000)。零改 src |
| [cache_latency_bench_rrf_round3.log.md](cache_latency_bench_rrf_round3.log.md) | `In Progress` (RRF Round 3 完成 / 进 Round 4, 2026-06-01) | **weighted_rrf 调优 Round 3 = LeanRRF（framework 开销消除，唯一真 rrf-specific 新 backend）**：roofline 定 gap 1.70ms(>1ms→做)。`LeanRRFBackend(LeanSearchBackend)` 复用 `_lean_bucket`、override `_search_weighted_rrf`：稳态直接 GEMV→argsort→RRF→topk(1) 跳 wrapper/scatter。**RRF search 4.73→3.82ms / p95 7.16→5.66**（与 weighted_sum lean 3.7ms 一致）。等价 self-included **rrf_score_abs_diff_max=0.0**(RRF score 离散 rank 函数，prenorm 不改 rank→bit-equal)，单测 4/4。审查 APPROVE(RRF FUSION FAITHFUL/BIT EQUAL vs PRENORM/WEIGHTED_SUM UNAFFECTED/SRC UNTOUCHED YES，0 finding，300 seeds 复跑)。零改 src |
| [cache_latency_bench_rrf_round4.log.md](cache_latency_bench_rrf_round4.log.md) | `In Progress` (RRF Round 4 完成, 2026-06-01) | **weighted_rrf 调优 R4 = batched keybuilder 复用**（与 fusion 正交，零新代码）：run_rrf `--batched` 叠加 R4 keybuilder。build 1.198→0.465ms(2.6×)，total 5.66→4.86ms，**hit_rate 0.3852 不变**(query_key bit-equal→检索不变)。审查独立验 batched 对 RRF query_key bit-equal。零改 src |
| [cache_latency_bench_rrf_round5.log.md](cache_latency_bench_rrf_round5.log.md) | `In Progress` (RRF Round 5 完成, 2026-06-01) | **weighted_rrf 调优 R5 = RrfReleaseVision（MRO 零代码组合）**：`RrfReleaseVisionBackend(ReleaseVisionBackend, LeanRRFBackend)` 纯多继承组合(release/guard←ReleaseVision, _search_weighted_rrf←LeanRRF)。全栈 search 3.81/build 0.495/**total 4.88ms**/释放 ~1GB/hit_rate 不变。单测 4/4。审查 APPROVE(MRO SAFE/GUARD COVERS RRF FALLBACK/SRC UNTOUCHED YES)。释放后 lean_rrf 用 keybuilder query+_mat 不读释放 entry→安全。零改 src |
| [cache_latency_bench_rrf_final_report.log.md](cache_latency_bench_rrf_final_report.log.md) | `In Progress` (RRF 6 轮收官, 2026-06-01) | **weighted_rrf 最终总报告（6 轮 + 对标 weighted_sum）**：复用 weighted_sum backend ~90% + 唯一 rrf-specific 新增 LeanRRF ~10%，**stock 32.88→全栈 4.88ms(6.7×)**。**检索侧(search 3.81+build 0.495)与 weighted_sum(4.05)持平**(印证 owner 'RRF 不差于 sum')，total 差额纯在 kinematic judge(0.38 vs threshold 0.01)非检索。等价靠 winner-id parity(RRF 几何界失效)；fp32-ONLY 更硬(dtype sweep RRF 翻档 4-12× 于 sum)。每轮独立审查 APPROVE，0 winner 翻档，零改 src |
