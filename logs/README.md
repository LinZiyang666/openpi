...
|------|--------|-------------|
| [cache_gate_design_brainstorm.log.md](cache_gate_design_brainstorm.log.md) | `Design Only` (Understand 阶段头脑风暴, 2026-07-02, upd 2026-07-03) | **L0 文档**: gate（CP1"搜不搜"决策）设计方案谱系头脑风暴 — 现状核实（gate skip 只省 search+judge+fetch ≈3.5–4.2ms 优化后；build/D2H 无条件执行；docs §5.1 / tutorial §3.1 的 gate/build 顺序与源码不符待修）+ 价值重估（V1 省延迟=每步省 search+judge+fetch 随库/吞吐/远程放大、V2 保 SR、V3=SR–算力预算旋钮；oracle[GT 事后偏差 deviate_score, 线上不可得, 仅上限/离线标签, 非 gate 输入] vs 盲拦截：全体口径同预算 +8~12pp SR / 同 SR 省 ~40% 预算，保守口径与 periodic 最优打平）+ 方案谱系：G0a hook 补丁 / **G0b 离线信号预测力研究(P0)** / A 静态先验(步位置/任务/库覆盖) / B 时序动态元信号(**B3 复用债务门**/停滞检测；B1 视觉变化/B2 动作动力学已剔除——与 verdict kinematic 因子重合) / C 双目标标定 / D 学习型 MLP + 双模型对照(整合 Opus 独立方案 + 期望值判据统一 V1+V2)。首推 G0b + B3（exp 层 ClientControlledGate 原型，零 src 改动）；验收及格线 = 同预算打败 periodic |
